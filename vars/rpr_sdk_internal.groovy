...
def executeUnitTests(String osName, String asicName, Map options)
{
    dir("RadeonProRenderSDK") {
        withNotifications(title: options["stageName"], options: options, logUrl: "${env.BUILD_URL}", configuration: NotificationConfiguration.DOWNLOAD_SOURCE_CODE_REPO) {
    ...
    try {
        GithubNotificator.updateStatus("Test", options['stageName'], "in_progress", options, NotificationConfiguration.EXECUTE_UNIT_TESTS, "${env.BUILD_URL}")
        ...
        utils.publishReport(this, "${env.BUILD_URL}", "${asicName}-${osName}_failures", "report.html", "${STAGE_NAME}_failures", "${STAGE_NAME}_failures", options.storeOnNAS, ["jenkinsBuildUrl": env.BUILD_URL, "jenkinsBuildName": currentBuild.displayName])
...
def executeTests(String osName, String asicName, Map options)
{
    ...
    try {
        ...
        if (e instanceof ExpectedExceptionWrapper) {
            ...
            GithubNotificator.updateStatus("Test", options['stageName'], "failure", options, errorMessage, "${env.BUILD_URL}")
...
def executeBuildWindows(Map options) {
    withNotifications(title: "Windows", options: options, logUrl: "${env.BUILD_URL}/artifact/Build-Windows.log",
        artifactUrl: "${env.BUILD_URL}/artifact/binCoreWin64.zip", configuration: NotificationConfiguration.BUILD_PACKAGE) {
