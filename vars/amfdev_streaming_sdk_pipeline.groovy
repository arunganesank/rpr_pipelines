    ...
    try {
        ...
        withNotifications(title: options["stageName"], options: options, configuration: NotificationConfiguration.INSTALL_PLUGIN) {
            timeout(time: "5", unit: "MINUTES") {
                ...
                if (options.multiconnectionConfiguration.android_client.any { options.parsedTests.contains(it) } || options.parsedTests == "regression.2.json~") {
        ...
        if (options.multiconnectionConfiguration.second_win_client.any { options.tests.contains(it) } || options.parsedTests == "regression.1.json~" || options.parsedTests == "regression.2.json~") {
...
def executeTests(String osName, String asicName, Map options) {
    ...
    try {
        ...
        if (osName == "Windows") {
            ...
            if (options.multiconnectionConfiguration.second_win_client.any { options.parsedTests.contains(it) } || options.parsedTests == "regression.1.json~" || options.parsedTests == "regression.2.json~") {
                    ...
                    if (it.endsWith(game)) {
                        ...
                        dir(testName.replace("testResult-", "")) {
                            if (it.contains("Android")) {
                                ...
                                if (options.multiconnectionConfiguration.second_win_client.any { testGroup -> it.contains(testGroup) } || testName.contains("regression.1.json~") || testName.contains("regression.2.json~")) {
                                    try {
